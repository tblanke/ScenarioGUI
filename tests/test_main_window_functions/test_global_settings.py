...
def test_global_settings(qtbot):  # noqa: PLR0915
    ...
    # init gui window
    load_config(
        Path(".").absolute().joinpath("./tests/gui_config.ini")
        if Path(".").absolute().joinpath("./tests/gui_config.ini").exists()
        else Path("..").absolute().joinpath("./tests/gui_config.ini")
    )
    ...
    assert globs.get_path_for_file(globs.path.joinpath("./ScenarioGUI/gui_classes/gui_structure_classes"), "gui_config.ini") in [
        globs.path.parent.joinpath("." if "tests" in f"{Path('.').absolute()}" else "./examples"),
        globs.path.parent.joinpath("." if "tests" in f"{Path('.').absolute()}" else "./tests"),
    ]
    ...
    [
        check_font(widget, globs.FONT_SIZE, globs.FONT, isinstance(widget, QtW.QPushButton))
        for widget in main_window.gui_structure.flex_option.frame.children()
    ...
    assert version == globs.VERSION

    load_config(path.joinpath("gui_config_with_version.ini"))

    assert globs.VERSION == "0.1.1.1.1"
