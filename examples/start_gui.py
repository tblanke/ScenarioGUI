...
from scenario_gui.global_settings import GUI_NAME, FILE_EXTENSION
from scenario_gui.gui_classes.gui_structure import GuiStructure, IntBox, Category, Page, Hint, ResultText, Aim
from scenario_gui.gui_classes.translation_class import Translations
import scenario_gui.global_settings as global_vars
...
class GUI(GuiStructure):
    def __init__(self, default_parent: QtW.QWidget, translations: Translations):
        ...
        self.int_a = IntBox(label='a', default_value=2, minimal_value=0, maximal_value=200, category= self.category_inputs)
        self.int_b = IntBox(label='b', default_value=2, minimal_value=0, maximal_value=200, category= self.category_inputs)
...
def run(path_list=None):  # pragma: no cover
    ...
    from scenario_gui.global_settings import VERSION
    from scenario_gui.gui_classes.gui_combine_window import MainWindow
    from scenario_gui.gui_classes.translation_class import Translations
    ...
    if path_list is not None:
        main_window.filename = ([path for path in path_list if path.endswith(f'.{FILE_EXTENSION}')][0], 0)
